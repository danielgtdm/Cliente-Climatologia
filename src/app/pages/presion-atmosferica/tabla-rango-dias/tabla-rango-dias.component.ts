...
export class TablaRangoDiasComponent implements OnInit {
  ...
  public exportarExcel() {
    if(this.listaRegistros.length == 0){
      ...
      this.excelService.generatePresionAtmosfericaExcel(this.listaRegistros);
    }
  }
  ...
  private viewDataTable(listaRegistros: Registro[]) {
    ...
    for (let i = 0; i < registros.length; i++) {
      ...
      data.push({
        'Fecha': fecha,
        '08:30 hrs': presionAtmosferica.h0830 != null ? `${presionAtmosferica.h0830}` : 'No Registrado',
        '14:00 hrs': presionAtmosferica.h1400 != null ? `${presionAtmosferica.h1400}` : 'No Registrado',
        '18:00 hrs': presionAtmosferica.h1800 != null ? `${presionAtmosferica.h1800}` : 'No Registrado'
      });
    }
